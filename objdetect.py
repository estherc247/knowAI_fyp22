...
def display_image(image):
  ...
  plt.grid(False)
  plt.imshow(image)
  plt.savefig('public/images/objd_output/object_d.png')


def download_and_resize_image(image_data, new_width=256, new_height=256,
                              display=False):
  filename='public/images/objd_output/ori_object_d.png'
  pil_image = Image.open(io.BytesIO(image_data))
  pil_image = ImageOps.fit(pil_image, (new_width, new_height), Image.ANTIALIAS)
  pil_image_rgb = pil_image.convert("RGB")
  pil_image_rgb.save(filename, quality=90)
...
def draw_boxes(image, boxes, class_names, scores, max_boxes=10, min_score=0.1):
  ...
  try:
    #font = ImageFont.load("public/fonts/Roboto-Bold.ttf")
    font = ImageFont.truetype("public/fonts/Roboto-Bold.ttf", 40)
  except IOError:
